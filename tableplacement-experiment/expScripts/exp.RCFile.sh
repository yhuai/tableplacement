...
do
  ...
  do
    ...
    do
      ./exp.strace.read.RCFile.sh $EXP $ROW_GROUP_SIZE $IO_BUFFER_SIZE $READ_COLUMN_STR $LOGDIR > $LOGDIR/strace.$RCFILE_PREFIX.$FILE_PREFIX.c$ROW_COUNT.rg$ROW_GROUP_SIZE.io$IO_BUFFER_SIZE.$READ_COLUMN_STR.log 2>&1
    done
  done
done
